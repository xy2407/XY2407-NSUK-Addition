...
final class HeaderSectionRenderer {
    ...
    static String buildTitleExtra(Minecraft mc) {
        ...
        String seasonInfo = getSeasonExtra(mc);
        return currentDay + "天 " + weekDay + seasonInfo;
    }

    private static String getSeasonExtra(Minecraft mc) {
        if (mc.level == null) return "";
        try {
            SolarTerm term = EclipticUtil.getNowSolarTerm(mc.level);
            if (term == null || !term.isValid()) return "";
            return "  " + term.getTranslation().getString();
