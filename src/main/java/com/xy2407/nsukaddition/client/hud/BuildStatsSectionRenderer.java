...
/** 侧边栏建筑统计和材料储备双栏渲染，带缓存避免重复计算。 */
final class BuildStatsSectionRenderer {
    private static final Cache CACHE = new Cache();

    private BuildStatsSectionRenderer() {}

    static int render(PopupLayoutManager.Rect r, Minecraft mc, Matrix4f matrix, int startY,
        ...
        int rightContentW = r.width() - padX * 2 - leftWidth - SidebarLayout.DIVIDER_TEXT_GAP;
        int rightColGap = 14;
        int rightColInset = 4;
        ...
        textRenderer.drawText(mc.font, CACHE.leftTitle, leftX, startY,
                SidebarHudLayer.TEXT_PRIMARY, false, matrix);
        textRenderer.drawText(mc.font, CACHE.rightTitle, rightContentX, startY,
                SidebarHudLayer.TEXT_PRIMARY, false, matrix);

        int rowY = startY + titleH;
        for (int i = 0; i < rowCount; i++) {
            if (i < leftRowCount) {
                renderRow(mc, matrix, textRenderer, CACHE.leftRows.get(i), leftX, rowY, leftContentW);
            }
            if (i < rightCol1Count) {
                renderRow(mc, matrix, textRenderer, CACHE.rightRows.get(i), rightCol1ContentX, rowY, rightColContentW);
            }
            int col2Index = i + rightCol1Count;
            if (col2Index < CACHE.rightRows.size()) {
                renderRow(mc, matrix, textRenderer, CACHE.rightRows.get(col2Index), rightCol2ContentX, rowY, rightColContentW);
            }
            rowY += lineH;
        }
        ...
        for (int y = dashTop; y < dashBottom; y += 5) {
            int dashHeight = Math.min(2, dashBottom - y);
            rectRenderer.fill(matrix, dividerX, y, 1, dashHeight, SidebarHudLayer.DIVIDER);
        }
    ...
    private static void renderRow(Minecraft mc, Matrix4f matrix, BatchTextRenderer textRenderer,
                                  RowCache row, int x, int y, int width) {
        textRenderer.drawText(mc.font, row.label, x, y, SidebarHudLayer.TEXT_SECONDARY, false, matrix);
