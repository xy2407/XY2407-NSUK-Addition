...
public final class RestaurantMaidHireScreenOpener {
    ...
    private static ModularUI createUi(RestaurantMaidHireResponsePacket p) {
        ...
        titleBar.addChild(label(Component.translatable("gui.xy2407_nsuk_addition.restaurant.maid_hire_title"), Horizontal.CENTER, 0xFFFFFFFF, tbH, TextWrap.HIDE));
        panel.addChild(titleBar);

        long hiredCount = p.candidates().stream().filter(RestaurantMaidHireResponsePacket.MaidCandidate::hired).count();
        panel.addChild(label(Component.translatable("gui.xy2407_nsuk_addition.restaurant.maid_count_line", hiredCount),
                Horizontal.LEFT, 0xFFE0E0FF, 12, TextWrap.HIDE));

        TextField search = new TextField();
        search.setAnyString();
        search.setText("");
        search.textFieldStyle(s -> s.placeholder(Component.translatable("gui.xy2407_nsuk_addition.restaurant.search_maid")).textColor(0xFFFFFFFF).textShadow(true));
    ...
    private static void refreshList(ScrollerView scroller, List<RestaurantMaidHireResponsePacket.MaidCandidate> candidates,
        ...
        if (candidates.isEmpty() && q.isEmpty()) {
            scroller.addScrollViewChild(label(Component.translatable("gui.xy2407_nsuk_addition.restaurant.maid_none_found"),
    ...
    private static UIElement createRow(RestaurantMaidHireResponsePacket.MaidCandidate candidate, boolean hired, BlockPos pos) {
        ...
        toggle.setText(Component.translatable(hired
                ? "gui.xy2407_nsuk_addition.restaurant.fire_maid"
                : "gui.xy2407_nsuk_addition.restaurant.hire_maid"));
