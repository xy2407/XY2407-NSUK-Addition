...
import com.xy2407.nsukaddition.common.mining.PickaxeMiningCalculator;
...
public final class MiningControlBoxUiRoot extends UIElement {
    ...
    private Button restockOnBtn;
    private Button restockOffBtn;
    private ClientAutoRestockCache.StateChangeListener restockListener;
    ...
    private UIElement titleBar(LayoutMetrics m) {
        ...
        if (boxPos != null) {
            ...
            restockOnBtn = new Button();
            restockOnBtn.setText(Component.translatable("gui.xy2407_nsuk_addition.on"));
            restockOnBtn.setActive(!on);

            restockOffBtn = new Button();
            restockOffBtn.setText(Component.translatable("gui.xy2407_nsuk_addition.off"));
            restockOffBtn.setActive(on);

            restockOnBtn.setOnClick(event -> {
                ClientAutoRestockCache.set(boxPos, true);
                PacketDistributor.sendToServer(new AutoRestockTogglePacket(boxPos, true));
                restockOnBtn.setActive(false);
                restockOffBtn.setActive(true);
            });
            restockOffBtn.setOnClick(event -> {
                ClientAutoRestockCache.set(boxPos, false);
                PacketDistributor.sendToServer(new AutoRestockTogglePacket(boxPos, false));
                restockOnBtn.setActive(true);
                restockOffBtn.setActive(false);
            });

            restockListener = (pos, enabled) -> {
                if (pos.equals(boxPos.immutable())) {
                    restockOnBtn.setActive(!enabled);
                    restockOffBtn.setActive(enabled);
                }
            };
            ClientAutoRestockCache.addListener(restockListener);
            ...
            bar.addChild(anchor);
            bar.addChild(restockOnBtn);
            bar.addChild(restockOffBtn);
        }
    ...
    private void refreshDynamicElements() {
        ...
        yLevelLabel.setText(Component.translatable("gui.xy2407_nsuk_addition.mining.y_level", packet.currentYLevel()));
        statusLabel.setText(statusLine());

        int maxTicks = packet.maxWorkTicks() > 0 ? packet.maxWorkTicks() : PickaxeMiningCalculator.BASE_TICKS_PER_LAYER;
        ...
        progressText.setText(Component.translatable("gui.xy2407_nsuk_addition.mining.progress", packet.workTicks(), maxTicks));
    ...
    private void demolish() {
        if (restockListener != null) {
            ClientAutoRestockCache.removeListener(restockListener);
            restockListener = null;
        }
    ...
    private void bindContainer() {
        ...
        if (mc.player != null) {
            mc.player.displayClientMessage(
                    Component.translatable("gui.xy2407_nsuk_addition.mining.auto_detect"), false);
        }
    }
