...
public final class ForeignTradeControlBoxScreenOpener {
    ...
    private static ModularUI createUi(ForeignTradeControlBoxOpenResponsePacket packet) {
        ...
        panel.addChild(autoRestockRow(packet, m));
    ...
    private static UIElement autoRestockRow(ForeignTradeControlBoxOpenResponsePacket packet, LayoutMetrics m) {
        ...
        boolean enabled = packet.autoRestock();
        Label statusLabel = new Label();
        statusLabel.setText(Component.translatable(enabled
                ? "gui.xy2407_nsuk_addition.foreign_trade.auto_restock_on"
                : "gui.xy2407_nsuk_addition.foreign_trade.auto_restock_off"));
        statusLabel.setOverflowVisible(false);
        statusLabel.layout(layout -> { layout.flex(1); layout.height(m.actionHeight()); });
        statusLabel.textStyle(style -> style
                .textColor(enabled ? 0xFF2E7D32 : TEXT_STATUS_COLOR)
                .textShadow(false)
        ...
        toggle.setText(Component.translatable("gui.xy2407_nsuk_addition.foreign_trade.auto_restock_toggle"));
        toggle.setOnClick(event -> action(packet, ForeignTradeControlBoxActionPacket.Action.TOGGLE_AUTORESTOCK));
        toggle.textStyle(style -> style.textWrap(TextWrap.HOVER_ROLL).rollSpeed(TEXT_ROLL_SPEED).textColor(TEXT_COLOR).textShadow(false));
        toggle.buttonStyle(style -> style
                .baseTexture(new GuiTextureGroup(new ColorRectTexture(enabled ? 0xFFC8E6C9 : BUTTON_BASE), new ColorBorderTexture(-1, BUTTON_BORDER)))
                .hoverTexture(new GuiTextureGroup(new ColorRectTexture(enabled ? 0xFFB2DFDB : BUTTON_HOVER), new ColorBorderTexture(-1, BUTTON_BORDER)))
                .pressedTexture(new GuiTextureGroup(new ColorRectTexture(BUTTON_PRESSED), new ColorBorderTexture(-1, BUTTON_BORDER)))
        );
        toggle.layout(layout -> { layout.width(80); layout.height(m.actionHeight()); layout.flexShrink(0); });
        row.addChild(toggle);
