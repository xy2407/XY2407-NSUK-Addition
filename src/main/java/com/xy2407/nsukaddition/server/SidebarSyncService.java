...
public final class SidebarSyncService {
    ...
    private static final long SYNC_INTERVAL = 100L;
    ...
    private static final class CachedTaskMaterials {
        ...
        final List<SidebarSyncPacket.MaterialEntry> availEntries;
        CachedTaskMaterials(int lastBlockIndex, List<SidebarSyncPacket.MaterialEntry> req,
                            List<SidebarSyncPacket.MaterialEntry> avail) {
            this.lastBlockIndex = lastBlockIndex;
            this.reqEntries = req;
            this.availEntries = avail;
        }
    }
    ...
    private static List<SidebarSyncPacket.BuildTaskData> collectBuildTasks(
        ...
        try {
            ...
            for (BuildingTaskData t : cityTasks) {
                String st = t.status();
                if ("completed".equals(st) || "interrupted".equals(st)) {
                    MATERIAL_CACHE.remove(t.citizenId());
                    continue;
                }
                ...
                CachedTaskMaterials cached = MATERIAL_CACHE.get(t.citizenId());
                List<SidebarSyncPacket.MaterialEntry> reqEntries;
                List<SidebarSyncPacket.MaterialEntry> availEntries;
                if (cached != null && cached.lastBlockIndex == t.currentBlockIndex()) {
                    reqEntries = cached.reqEntries;
                    availEntries = cached.availEntries;
                } else {
                    ...
                    availEntries = toTaskEntries(totalRequired, available);
                    MATERIAL_CACHE.put(t.citizenId(), new CachedTaskMaterials(t.currentBlockIndex(), reqEntries, availEntries));
                }
