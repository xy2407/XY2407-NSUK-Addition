...
public final class OreVeinDistributionService {
    ...
    private static final long SALT_PRISMARINE = 0x507269736D617269L;
    ...
    private static final int PRISMARINE_THRESHOLD = 328;
    ...
    private static final ConcurrentHashMap<String, ConcurrentHashMap<Long, Set<Long>>> veinIdToChunks = new ConcurrentHashMap<>();
    ...
    public static OreVeinChunkData getVeinAt(ServerLevel level, int chunkX, int chunkZ) {
        ...
        if (!isVeinCenter(seed, chunkX, chunkZ, level.dimension())) {
            return tryPrismarineVein(level, chunkX, chunkZ, seed, veinCache, blockedChunks);
        }
        ...
        ConcurrentHashMap<Long, Set<Long>> dimVeinIndex = veinIdToChunks.computeIfAbsent(dimKey, k -> new ConcurrentHashMap<>());
        Set<Long> chunkSet = dimVeinIndex.computeIfAbsent(veinId, k -> ConcurrentHashMap.newKeySet());
    ...
    public static Set<Long> getVeinChunks(ServerLevel level, int chunkX, int chunkZ) {
        ...
        String dimKey = level.dimension().location().toString();
        ConcurrentHashMap<Long, Set<Long>> dimVeinIndex = veinIdToChunks.get(dimKey);
        if (dimVeinIndex == null) return Collections.emptySet();
        Set<Long> chunks = dimVeinIndex.get(data.veinId());
        return chunks != null ? Collections.unmodifiableSet(chunks) : Collections.emptySet();
    }

    private static OreVeinChunkData tryPrismarineVein(ServerLevel level, int chunkX, int chunkZ, long seed,
                                                       ConcurrentHashMap<Long, OreVeinChunkData> veinCache,
                                                       Set<Long> blockedChunks) {
        if (level.dimension() != Level.OVERWORLD) return null;
        if (!isOceanBiome(level, chunkX, chunkZ)) return null;
        if (!isPrismarineCenter(seed, chunkX, chunkZ)) return null;

        OreVeinType oreType = OreVeinType.PRISMARINE;
        int targetSize = selectVeinSize(seed, chunkX, chunkZ);
        Set<Long> veinChunks = growVein(seed, chunkX, chunkZ, targetSize, veinCache, blockedChunks);
        if (veinChunks.isEmpty()) return null;
    ...
    private static boolean isOceanBiome(ServerLevel level, int chunkX, int chunkZ) {
        BlockPos center = new BlockPos((chunkX << 4) + 8, 63, (chunkZ << 4) + 8);
        Holder<Biome> biome = level.getBiome(center);
        return biome.is(BiomeTags.IS_OCEAN);
    }

    private static boolean isPrismarineCenter(long worldSeed, int cx, int cz) {
        long h = hashPos(worldSeed, cx, cz, SALT_PRISMARINE);
        return (h & 0xFFF) < PRISMARINE_THRESHOLD;
    }
