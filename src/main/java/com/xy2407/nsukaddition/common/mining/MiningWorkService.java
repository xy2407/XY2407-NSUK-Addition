...
public final class MiningWorkService {
    ...
    private static void tickBox(ServerLevel level, MiningBoxManager manager, MiningBoxData data) {
        ...
        ItemStack activePickaxe = getActivePickaxe(level, boxPos);
        int requiredTicks = PickaxeMiningCalculator.calculateLayerTicks(activePickaxe, level.registryAccess());

        if (data.workTicks() < requiredTicks) {
            return;
        }
        ...
        switch (result.status()) {
            ...
            case CONTAINER_FULL -> {
                data.setWorkTicks(Math.max(0, requiredTicks - 1));
    ...
    /** 获取采矿盒当前可用的镐子（用于计算挖掘速度）。 */
    private static ItemStack getActivePickaxe(ServerLevel level, BlockPos boxPos) {
        BlockEntity be = level.getBlockEntity(boxPos);
        if (be instanceof MiningControlBoxBlockEntity box) {
            return box.getActivePickaxe();
        }
