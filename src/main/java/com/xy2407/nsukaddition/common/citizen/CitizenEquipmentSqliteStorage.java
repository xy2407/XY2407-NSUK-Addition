...
public final class CitizenEquipmentSqliteStorage {
    ...
    /** 内存缓存，避免每次实体加载都查库 */
    private static final java.util.concurrent.ConcurrentMap<UUID, Map<EquipmentSlot, ItemStack>> CACHE = new java.util.concurrent.ConcurrentHashMap<>();
    ...
    public static void save(ServerLevel level, UUID citizenUuid, Map<EquipmentSlot, ItemStack> equipment) {
        ...
        Map<EquipmentSlot, ItemStack> cacheEntry = new EnumMap<>(equipment);
        CACHE.put(citizenUuid, cacheEntry);
    ...
    public static Map<EquipmentSlot, ItemStack> load(ServerLevel level, UUID citizenUuid) {
        if (level == null || citizenUuid == null) return new EnumMap<>(EquipmentSlot.class);
        Map<EquipmentSlot, ItemStack> cached = CACHE.get(citizenUuid);
        if (cached != null) {
        ...
        try {
            ...
            if (!result.isEmpty()) {
                CACHE.put(citizenUuid, new EnumMap<>(result));
            }
