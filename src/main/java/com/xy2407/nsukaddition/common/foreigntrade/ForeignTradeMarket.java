...
public final class ForeignTradeMarket {
    ...
    public record MarketEntry(String itemId, int count, double buyPrice, double sellPrice, String category) {}
    ...
    public static void refresh() {
        ...
        for (var def : ForeignTradeConfig.getEntries()) {
            ...
            double buyPrice = Math.round(def.buy() * (1.0 + buyFluc) * 10.0) / 10.0;
            double sellPrice = Math.round(def.sell() * (1.0 + sellFluc) * 10.0) / 10.0;
