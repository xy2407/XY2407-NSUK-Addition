...
public final class ModFarmCropRegistry {
    ...
    public static int cropMapSize() {
        return CROP_MAP.size();
    }
    ...
    private static void register(String cropId, String seedItemId, String plantBlockId,
        ...
        if (seedId == null || plantId == null) {
            NsukAddition.LOGGER.warn("[农田注册] 跳过 {} : ID解析失败 seed={} plant={}", cropId, seedItemId, plantBlockId);
            return;
        }

        if (!BuiltInRegistries.ITEM.containsKey(seedId)) {
            NsukAddition.LOGGER.warn("[农田注册] 跳过 {} : 种子物品不存在 seed={}", cropId, seedItemId);
            return;
        }
        if (!BuiltInRegistries.BLOCK.containsKey(plantId)) {
            NsukAddition.LOGGER.warn("[农田注册] 跳过 {} : 作物方块不存在 plant={}", cropId, plantBlockId);
            return;
        }
