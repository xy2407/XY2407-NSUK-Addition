...
public final class AutoRestockService {
    ...
    /** 餐厅自动补货：根据玩家选择的菜品反查原料，从仓库补货到 input 容器；产出不入库（实时被 NPC 消费）。 */
    public static void restockRestaurantInputs(ServerLevel level, BlockPos pos) {
        ...
        final int targetStockPerInput = 1;

        java.util.LinkedHashMap<Ingredient, Integer> needed = new java.util.LinkedHashMap<>();
        for (String itemId : cookItems) {
            ...
            for (Ingredient ing : recipe.ingredients()) {
                ...
                needed.merge(ing, targetStockPerInput, Math::max);
            }
        }
        if (needed.isEmpty()) return;

        // 收集 input 容器中现有物品，按 Ingredient 匹配统计
        for (var entry : needed.entrySet()) {
            ...
            ItemStack extracted = extractIngredientFromWarehouse(level, warehousePos, ing, shortage);
            if (extracted.isEmpty()) continue;
    ...
    /** 统计 input 容器中匹配 Ingredient 的物品数量。 */
    ...
    /** 从仓库提取匹配 Ingredient 的物品，最多 count 个。 */
    private static ItemStack extractIngredientFromWarehouse(ServerLevel level, BlockPos warehousePos,
    ...
    public static void restockFarmlandInputs(ServerLevel level, BlockPos pos) {
        ...
        if (boneMealShortage > 0) {
            ...
            ItemStack extracted = LogisticsWarehouseInventoryService.extract(level, warehousePos, template, boneMealShortage);
            if (!extracted.isEmpty()) {
                ItemStack leftover = insertIntoContainers(level, chests, extracted);
                if (!leftover.isEmpty()) {
                    LogisticsWarehouseInventoryService.insert(level, warehousePos, leftover);
        ...
        for (BlockPos chest : chests) {
            ...
            for (GenericContainerAccess.SlotSnapshot slot : GenericContainerAccess.snapshotSlots(level, chest)) {
                ...
                if (seed != null && stack.is(seed)) {
                    ...
                    ItemStack remaining = LogisticsWarehouseInventoryService.insert(level, warehousePos, toMove);
