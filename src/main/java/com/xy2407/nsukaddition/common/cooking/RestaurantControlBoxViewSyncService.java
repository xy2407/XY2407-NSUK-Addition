...
/** 餐厅控制箱视图同步服务，按变更检测向附近玩家推送状态更新。 */
@SuppressWarnings("null")
public final class RestaurantControlBoxViewSyncService {
    ...
    private RestaurantControlBoxViewSyncService() {
    }

    public static void syncStatusIfChanged(ServerLevel level, RestaurantBoxData data) {
        if (level == null || data == null || data.boxPos() == null) return;
        ...
        if (previous != null) {
            if (snapshot.equals(previous.snapshot())) return;
            ...
            if (!urgent && gameTime - previous.syncedAt() < MIN_SYNC_INTERVAL_TICKS) return;
        }
        ...
        PacketDistributor.sendToPlayersNear(
                ...
                SYNC_RADIUS,
                RestaurantControlBoxViewUpdatePacket.from(RestaurantControlBoxOpenResponsePacket.from(RestaurantControlBoxService.buildView(level, pos))));
    }
